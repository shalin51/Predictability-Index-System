...
export async function handleApiRequest(request: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (request.method.toUpperCase() === 'POST' && (['/setup-sheet-imports/preview', '/production-run-imports/preview', '/material-imports/preview'].includes(normalizedPath) || /^\/data-transfer\/[^/]+\/(import|validate)$/.test(normalizedPath))) {
    const contentType = request.headers.get('content-type')?.toLowerCase() ?? '';
