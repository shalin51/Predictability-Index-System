...
export const libraryConfigs = {
  ...
  'machine-setup-profiles': {
    columns: ['id', 'machineId', 'machineCode', 'profileCode', 'profileName', 'parameters', 'status', 'notes', 'updatedAt', 'createdAt'],
    createFields: [
      { key: 'machineId', label: 'Machine', required: true, type: 'select' },
      { key: 'profileCode', label: 'Profile Code', required: true },
      { key: 'profileName', label: 'Profile Name', required: true },
      { key: 'status', label: 'Status', type: 'select', allowedValues: ['active', 'inactive'] },
      { key: 'notes', label: 'Notes' },
    ],
    defaultOrderBy: 'machine_code, profile_code',
    displayName: 'Machine Setup Profile',
    idColumn: 'id',
    filterColumn: 'msp.machine_id',
    listSql: `SELECT msp.id, msp.machine_id AS "machineId", m.machine_code AS "machineCode", msp.profile_code AS "profileCode", msp.profile_name AS "profileName", msp.parameters, msp.status::text AS status, msp.notes, msp.updated_at AS "updatedAt", msp.created_at AS "createdAt" FROM machine_setup_profiles msp JOIN machines m ON m.id = msp.machine_id`,
    mutableColumns: ['machineId', 'profileCode', 'profileName', 'parameters', 'status', 'notes'],
    requiredFields: ['machineId', 'profileCode', 'profileName'],
    routeKey: 'machine-setup-profiles',
    searchColumns: ['m.machine_code', 'msp.profile_code', 'msp.profile_name'],
    selectOptions: { machineId: 'machines' },
    statusColumn: 'msp.status',
    tableName: 'machine_setup_profiles',
    uniqueChecks: [{ columns: ['profileCode'], message: 'profile_code must be unique' }],
  },
  'mold-zones': {
...
const extraLibraryConfigs: Record<string, LibraryEntityConfig> = {
  ...
  benchmarks: {
    columns: [], createFields: [
      ...
      { key: 'ballModel', label: 'Ball Model', required: true }, { key: 'testDate', label: 'Test Date', type: 'date' },
      { key: 'reportNumber', label: 'Report Number' }, { key: 'status', label: 'Status', type: 'select' }, { key: 'notes', label: 'Notes', type: 'textarea' },
    ], defaultOrderBy: 'benchmark_code, profile_version', displayName: 'Benchmark', idColumn: 'id',
    listSql: `SELECT id, benchmark_code AS "benchmarkCode", benchmark_name AS "benchmarkName", profile_version AS "profileVersion", ball_brand AS "ballBrand", ball_model AS "ballModel", test_date AS "testDate", report_number AS "reportNumber", status::text AS status, notes, updated_at AS "updatedAt", created_at AS "createdAt" FROM benchmark_profiles`,
    mutableColumns: ['benchmarkCode', 'benchmarkName', 'profileVersion', 'ballBrand', 'ballModel', 'testDate', 'reportNumber', 'status', 'notes'],
