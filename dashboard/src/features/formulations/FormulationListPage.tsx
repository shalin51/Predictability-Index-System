...
export function FormulationListPage({ onCreate, onOpen }: { onCreate: () => void; onOpen: (id: string) => void }) {
  ...
  const [filters, setFilters] = useState({ createdFrom: '', createdTo: '', materialId: '', search: '', status: 'all' });
  ...
  useEffect(() => {
    void listLibraryOptions('materials').then(setMaterials)
      .catch(() => undefined);
  }, []);
          ...
          <div style={formulationStyles.tableWrap}>
            <table style={formulationStyles.table}>
              <thead>
                <tr>
                  {[
                    ['formulationCode', 'Formulation Code'], ['formulationName', 'Formulation Name'], ['version', 'Version'], ['status', 'Status'], ['componentsTotal', 'Components Total'], ['updatedAt', 'Last Updated'],
                  ].map(([key, label]) => (
              ...
              <tbody>
                {table.pagedRecords.map((record) => (
                  <tr key={record.id}>
                    ...
                    <td style={formulationStyles.td}>{record.formulationName ?? '-'}</td>
